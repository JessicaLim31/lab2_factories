...
## Assignment
    ...
    def _cosine_similarity(self, embedding_a, embedding_b)-> float:
        ...
        if isinstance(embedding_b, list):
        ...
        cosine_email = dot_product / (norm_a * norm_b)
        normalized_score = (cosine_email + 1) / 2
        return float(normalized_score)
    
    
    
    
    def predict_emails(self, features: Dict[str, Any], stored_emails: List[Dict]) -> str:
        """Classify email into one of the stored ground truth using feature similarity"""
        threshold= 0.8
        
        email_embedding = features.get("email_embeddings_average_embedding", None)
        score_list = []
        
        # Find the best label from the stored ground truth
        for stored in stored_emails:
            if not stored.get("ground_truth") or not stored.get("embedding"):
                continue
            score = self._cosine_similarity(email_embedding, stored["embedding"])
            score_list.append((score, stored["ground_truth"]))
            
        if not score_list:
            return None, None
        
        best_score, best_label = max(score_list, key=lambda x: x[0])
